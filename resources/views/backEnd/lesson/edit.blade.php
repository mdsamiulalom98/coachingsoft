            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('lesson.update') }}" method="POST" class="row"
                                ...
                                <input type="hidden" value="{{ $edit_data->id }}" name="hidden_id">
                                    <div class="col-sm-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="form-control form-select select2 @error('course_id') is-invalid @enderror"
                                                id="course_id" name="course_id" required>
                                                <optgroup>
                                                    <option value="">Select..</option>
                                                    @foreach ($course_name as $category)
                                                        <option value="{{ $category->id }}" 
                                                            @if ($edit_data->course_id == $category->id) selected @endif>
                                                            {{ $category->title }}
                                                        </option>
                                                    @endforeach
                                                </optgroup>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="form-control form-select select2-multiple @error('chapter_id') is-invalid @enderror"
                                                id="chapter_id" name="chapter_id" data-placeholder="Choose ...">
                                                <optgroup>
                                                    <option value="">Select..</option>
                                                    @foreach ($subcategory as $chapter)
                                                        <option value="{{ $chapter->id }}" 
                                                            @if ($edit_data->chapter_id == $chapter->id) selected @endif>
                                                            {{ $chapter->title }}
                                                        </option>
                                                    @endforeach
                                                </optgroup>
...
<script>
    ...
    $("#course_id").on("change", function () {
        ...
        if (courseId) {
            $.ajax({
                type: "GET",
                url: "{{ url('ajax-product-subcategory') }}?course_id=" + courseId,
